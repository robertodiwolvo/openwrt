...
static int qca807x_set_downshift(struct phy_device *phydev, u8 cnt)
{
	...
	if (cnt > QCA807X_SMARTSPEED_RETRY_LIMIT_MAX ||
		(cnt < QCA807X_SMARTSPEED_RETRY_LIMIT_MIN &&
		 cnt != DOWNSHIFT_DEV_DISABLE))
		return -EINVAL;

	if (!cnt) {
		ret = phy_clear_bits(phydev, MII_NWAYTEST,
...
static int qca807x_gpio_get(struct gpio_chip *gc, unsigned int offset)
{
	...
	return phy_read_mmd(priv->phy, MDIO_MMD_AN, qca807x_gpio_get_reg(offset)) == QCA807X_GPIO_HIGH_VALUE;
}
...
static void qca807x_gpio_set(struct gpio_chip *gc, unsigned int offset, int value)
{
	...
	phy_write_mmd(priv->phy, MDIO_MMD_AN, qca807x_gpio_get_reg(offset),
				  value ? QCA807X_GPIO_HIGH_VALUE : QCA807X_GPIO_LOW_VALUE);
}
...
static int qca807x_read_status(struct phy_device *phydev)
{
	...
	/* Read the QCA807x PHY-Specific Status register, which indicates the
